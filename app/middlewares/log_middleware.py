...
from aiogram.types import TelegramObject, Message, CallbackQuery, Update, InlineQuery
...
class LogMiddleware(BaseMiddleware):
    ...
    @staticmethod
    def inline_query_bot_log(inline_query: InlineQuery, update: Update) -> None:
        logging.info(f'-------------------------------------------------------------------------------\n'
                     f'Пользователь: {inline_query.from_user.full_name}\n'
                     f'Отправил inline_query: {inline_query.query}\n'
                     f'Chat_ID(ID): {inline_query.}\n'
                     f'Ссылка: {callback.from_user.username}\n'
                     f'Время: {datetime.utcnow() + timedelta(hours=3)}\n'
                     f'Update id={update.update_id}'
                     )
    ...
    async def __call__(
        ...
        if isinstance(event, Update):
            ...
            if event.message is not None:
                ...
                self.inline_query_bot_log(event.inline_query, event)
